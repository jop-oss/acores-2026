...
const SC_CASELLES = (() => {
  ...
  [[0,0],[0,7],[0,14],[7,0],[7,14],[14,0],[14,7],[14,14]].forEach(([r,c]) => set(r,c,'TP'));

  // Doble paraula (DP) — diagonals interiors
  [[1,1],[2,2],[3,3],[4,4],[10,10],[11,11],[12,12],[13,13],
   [1,13],[2,12],[3,11],[4,10],[10,4],[11,3],[12,2],[13,1],
   [7,7]].forEach(([r,c]) => set(r,c,'DP'));

  // Triple lletra (TL)
  [[1,5],[1,9],[5,1],[5,5],[5,9],[5,13],
   [9,1],[9,5],[9,9],[9,13],[13,5],[13,9]].forEach(([r,c]) => set(r,c,'TL'));

  // Doble lletra (DL)
  [[0,3],[0,11],[2,6],[2,8],[3,0],[3,7],[3,14],
   [6,2],[6,6],[6,8],[6,12],[7,3],[7,11],
   [8,2],[8,6],[8,8],[8,12],[11,0],[11,7],[11,14],
   [12,6],[12,8],[14,3],[14,11]].forEach(([r,c]) => set(r,c,'DL'));
...
function scRenderRankingMini(p, modalitat) {
  ...
  el.innerHTML = jugadors.slice(0, 3).map((j, i) => `
...
async function scRenderRankingGlobal() {
  ...
  try {
    ...
    const llista = SC_ORDRE_JUGADORS
    ...
    if (el) el.innerHTML = '<div class="ranking-loading">Error carregant…</div>';
  }
}
...
async function scGetPuntsGlobals() {
  const pts = {};
  SC_ORDRE_JUGADORS.forEach(nom => { pts[nom] = 0; });
  try {
    const db = scrabbleGetDb();
    const [snapInd, snapEq] = await Promise.all([
      db.collection(SC_COL).doc(SC_IND_DOC).get(),
...
function scToggleJugador(nom) {
  if (scJugadorsSeleccionats.has(nom)) {
    if (scJugadorsSeleccionats.size <= 2) { alert('Mínim 2 jugadors.'); return; }
...
async function scIniciarPartidaIndividual() {
  ...
  const jugadorsOrdenats = SC_ORDRE_JUGADORS.filter(nom => scJugadorsSeleccionats.has(nom));
  if (jugadorsOrdenats.length < 2) { alert('Mínim 2 jugadors.'); return; }
  ...
  const jugadors = jugadorsOrdenats.map(nom => {
    ...
    return { nom, punts: 0, maEncriptada: scEncriptarMa(nom, ma), tornsPassats: 0 };
  });
...
function scRenderConfigEquips() {
  ...
  if (!cont) return;
  SC_ORDRE_JUGADORS.forEach(nom => { if (!scAssignacions[nom]) scAssignacions[nom] = 0; });
...
function scRenderConfigEquipsHTML() {
  ...
  cont.innerHTML = `
        ...
        return `
          <div style="display:flex;align-items:center;gap:.75rem;background:var(--bg2);border:1px solid var(--border);border-radius:10px;padding:.5rem .75rem">
            ...
            <div style="display:flex;gap:.35rem">
              ...
              ${Array.from({length: scNumEquips}, (_, i) => i+1).map(n => `
    ...
    <div style="background:var(--bg);border:1px solid var(--border);border-radius:10px;padding:.75rem;margin-bottom:1rem">
      ...
      ${Array.from({length: scNumEquips}, (_, i) => i+1).map(n => {
        const membres = SC_ORDRE_JUGADORS.filter(nom => scAssignacions[nom] === n);
...
async function scIniciarPartidaEquips() {
  ...
  if (equips.length < 2) { alert('Mínim 2 equips amb jugadors.'); return; }

  const totalJugadors = equips.reduce((s, e) => s + e.membres.length, 0);
  if (totalJugadors < 2) { alert('Mínim 2 jugadors en total.'); return; }
  ...
  const jugadors = equips.map(eq => {
    ...
    return { nom: eq.nom, membres: eq.membres, punts: 0, maEncriptada: scEncriptarMa(eq.nom, ma), tornsPassats: 0 };
  });
...
function scRenderRankingLateralHTML() {
  if (!scPartida) return '';
  const jugadors = [...(scPartida.jugadors || [])].sort((a, b) => b.punts - a.punts);
...
function scRenderMa() {
  ...
  el.innerHTML = scMaActual.map((fitxa, idx) => {
    if (!fitxa) return '';
...
function scCollocarFitxa(r, c) {
  ...
  if (comodin || lletra === '#') {
    ...
    if (!lletresDisp.includes(input)) { alert('Lletra no vàlida.'); return; }
    lletra = input;
    comodin = true;
  }
...
function scRetirarFitxa(r, c) {
  ...
  if (buit >= 0) {
    scMaActual[buit] = { lletra: fitxa.comodin ? '#' : fitxa.lletra, punts: fitxa.punts, comodin: fitxa.comodin };
  } else {
    scMaActual.push({ lletra: fitxa.comodin ? '#' : fitxa.lletra, punts: fitxa.punts });
  }
...
function scCalcularPunts(tauler, fitxesNoves) {
  ...
  if (taulerBuit) {
    ...
    if (!passaCentre) return { total: 0, paraules: [], valid: false, missatge: 'La primera jugada ha de passar per la casella central!' };
  }

  // Comprova connectivitat (les fitxes noves han de connectar amb les existents o ser la primera jugada)
  if (!taulerBuit) {
    const connecta = fitxesNoves.some(f => scTeVeiTauler(tauler, f.r, f.c, fitxesNoves));
    if (!connecta) return { total: 0, paraules: [], valid: false, missatge: 'Les fitxes han de connectar amb les ja col·locades!' };
  }

  // Construeix el tauler temporal
  const taulerTemp = tauler.map(fila => [...fila]);
  fitxesNoves.forEach(f => { taulerTemp[f.r][f.c] = { lletra: f.lletra, punts: f.punts, comodin: f.comodin }; });
  ...
  const paraulaPrincipal = horitzontal
    ...
    : scExtreurePararaulaV(taulerTemp, Math.min(...files), cols[0], fitxesNoves);
...
function scTeVeiTauler(tauler, r, c, fitxesNoves) {
  const veins = [[r-1,c],[r+1,c],[r,c-1],[r,c+1]];
...
function scExtreurePararaulaH(tauler, r, cInici, fitxesNoves, soloCreuada = false) {
  ...
  while (c2 < SC_MIDA && tauler[r]?.[c2]) {
    ...
    if (esNova) {
      ...
      else if (r === 7 && c2 === 7 && !tauler[r][c2].fixe) multiplicadorParaula *= 2;
    }
...
function scExtreurePararaulaV(tauler, rInici, c, fitxesNoves, soloCreuada = false) {
  let r = rInici;
  ...
  while (r2 < SC_MIDA && tauler[r2]?.[c]) {
    ...
    if (esNova) {
      ...
      else if (r2 === 7 && c === 7 && !tauler[r2][c].fixe) multiplicadorParaula *= 2;
    }
...
async function scAplicarJugada(resultat, eraValidacio) {
  ...
  const jugadors = scPartida.jugadors.map((j, i) => {
    ...
    return { ...j, punts: j.punts + resultat.total, maEncriptada: scEncriptarMa(jugadorNom, maNova), tornsPassats: 0 };
  });

  // Historial jugades
  const historialJugades = [...(scPartida.historialJugades || []),
    { nom: jugadorNom, paraules: resultat.paraules.map(p => p.paraula), punts: resultat.total, ts: Date.now() }
  ];
...
async function scDemanarValidacio(resultat, noTrobades) {
  ...
  try {
    ...
    mostraScreen('scrabble-inici');
    scCarregarInici();
  } catch(e) { console.error(e); alert('Error guardant la validació.'); }
}
...
async function scConfirmarCanvi() {
  ...
  scFitxesCanvi.forEach(idx => {
    ...
    if (fitxa) { sacNou.push(fitxa); maNova[idx] = null; }
  });
  ...
  scFitxesCanvi.forEach((idx) => { maNova[idx] = sacNou.shift(); });

  const jugadors = scPartida.jugadors.map((j, i) => {
    if (i !== jugadorIdx) return j;
    return { ...j, maEncriptada: scEncriptarMa(jugadorNom, maNova.filter(Boolean)) };
  });
...
async function scPassarTorn() {
  ...
  try {
    await scrabbleGetDb().collection(SC_COL).doc(docId).update({
      jugadors, tornIdx: nouTornIdx, ronda: novaRonda, tornTs: Date.now(),
...
async function scAdminPartida(modalitat) {
  ...
  if (opcio === '1') {
    ...
    await scrabbleGetDb().collection(SC_COL).doc(docId).update({ tornIdx: nouTornIdx, tornTs: Date.now() });
    alert('Torn passat.');
    scCarregarInici();
  } else if (opcio === '2') {
