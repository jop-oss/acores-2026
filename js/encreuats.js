...
function encMostrarClue() {
  ...
  // Trobar quin número de definició correspon
  const [r0, c0] = mot.cels[0];
  const num = encNumCelAny(r0, c0);
  if (!num) { clue.textContent = '—'; return; }

  // Buscar la definició que comença amb aquest número
  const defs = encDireccio === 'H' ? encPuzzle.hdef : encPuzzle.vdef;
  const def = defs.find(d => d.startsWith(`${num}.`) || d.startsWith(`${num}.~`));
  clue.textContent = def || '—';

  // Ressaltar la definició a la llista
  ...
  if (list) {
    const items = list.querySelectorAll('.enc-def-item');
    items.forEach(item => {
      if (item.textContent.trim().startsWith(`${num}.`)) {
...
function encClickDef(dir, idx) {
  ...
  // Trobar la primera casella que té aquest número
  const p = encPuzzle;
  if (p.tipus === 'ppcc') {
    for (let r = 0; r < p.rows; r++) {
      for (let c = 0; c < p.cols; c++) {
    ...
    for (let r = 0; r < N; r++) {
      for (let c = 0; c < N; c++) {
        ...
        const iH = (c === 0 || p.grid[r][c-1] === null) && c+1 < N && p.grid[r][c+1] !== null;
        const iV = (r === 0 || p.grid[r-1][c] === null) && r+1 < N && p.grid[r+1][c] !== null;
        if (iH || iV) {
          n++;
          if (n === num) {
            encCelActiva = { r, c };
