...
async function initDefaultDistances() {
  ...
  if (cached) {
    try {
      ...
      renderAll();
      return;
    } catch(e) { localStorage.removeItem(DIST_CACHE_KEY); }
  }
...
function resetToCapitalDist() {
  const cached = localStorage.getItem(DIST_CACHE_KEY);
  if (cached) {
    try { state.distances = JSON.parse(cached); } catch(e) { state.distances = {}; }
  } else {
    state.distances = {};
    initDefaultDistances();
    return;
...
function renderMarkers() {
  ...
  if (visible.length > 0 && visible.length < 50) {
    const bounds = visible.filter(r => r.lat && r.lon).map(r => [r.lat, r.lon]);
...
function filtered() {
  ...
  if (state.preu) {
    list = list.filter(r => (r.preu_google||'').includes(state.preu) || (r.preu_ta||'').includes(state.preu));
  }
  if (state.distMax > 0 && Object.keys(state.distances).length > 0) {
    list = list.filter(r => { const d = state.distances[r.id]; return !d || d.distKm <= state.distMax; });
  }
  list.sort((a, b) => {
    switch(state.sortBy) {
...
function renderList(list) {
  ...
  container.querySelectorAll('.rest-card').forEach(card => {
    ...
    initCarrusel(card);
    card.addEventListener('mouseenter', () => { const m = markers[id]; if (m) m.openPopup(); });
    card.addEventListener('mouseleave', () => { Object.values(markers).forEach(m => m.closePopup()); });
    card.querySelector('.rest-card-top').addEventListener('click', () => toggleDetail(id));
    card.querySelectorAll('.cuina-tag').forEach(tag => {
      tag.addEventListener('click', e => { e.stopPropagation(); addCuina(tag.dataset.cuina); });
    });
  });
}
...
function cardHtml(r) {
  ...
  const fotosG  = (r.fotos_google||[]).map(f => ({ url: f, font: 'Google' }));
  ...
  const editorBadge = r.editor ? `<span class="badge-editor ${['gold','silver','bronze'][r.editor-1]}">${['🥇','🥈','🥉'][r.editor-1]}</span>` : '';
  ...
  const cuinesHtml  = r.cuines.map(c => `<span class="cuina-tag" data-cuina="${c}">${c}</span>`).join('');
  ...
  const telBtn  = r.telefon     && isMobile ? `<a href="tel:${r.telefon}" class="action-btn" onclick="event.stopPropagation()">📞 Trucar</a>` : '';
  const waBtn   = r.google_maps && isMobile ? `<a href="https://wa.me/?text=${encodeURIComponent(r.nom+' '+r.google_maps)}" class="action-btn" onclick="event.stopPropagation()">💬 Compartir</a>` : '';
  const mapsBtn = r.google_maps ? `<a href="${r.google_maps}" target="_blank" class="action-btn primary" onclick="event.stopPropagation()">🗺️ Maps</a>` : '';
  const taBtn   = r.tripadvisor ? `<a href="${r.tripadvisor}" target="_blank" class="action-btn" onclick="event.stopPropagation()">🦉 TripAdvisor</a>` : '';
...
function toggleDetail(id) {
  ...
  if (!detail) return;
  if (detail.classList.contains('open')) { detail.classList.remove('open'); detail.innerHTML = ''; }
  else openDetail(id);
}
...
function initCarrusel(card) {
  ...
  let current = 0;
  setInterval(() => {
    fotos[current].classList.remove('active'); dots[current]?.classList.remove('active');
    current = (current + 1) % fotos.length;
    fotos[current].classList.add('active'); dots[current]?.classList.add('active');
  }, 3000);
  dots.forEach(dot => {
    ...
    try {
        ...
        data.rows[0].elements.forEach((el, i) => {
          if (el.status === 'OK') {
            state.distances[rests[i].id] = {
              dist: el.distance.text, temps: el.duration.text, distKm: el.distance.value/1000
            };
          }
        });
      }
      ...
      rests.forEach(r => {
        if (r.lat && r.lon) {
          ...
          state.distances[r.id] = { dist:`~${km.toFixed(1)} km`, temps:`~${Math.round(km/40*60)} min`, distKm:km };
        }
      });
    }
  }
...
function populateCuinesSelect() {
  ...
  [...cuines].sort().forEach(c => { const o = document.createElement('option'); o.value=c; o.textContent=c; sel.appendChild(o); });
}

function addCuina(cuina) {
  if (!state.cuines.includes(cuina)) { state.cuines.push(cuina); renderAll(); }
}

function removeCuina(cuina) {
  state.cuines = state.cuines.filter(c => c !== cuina); renderAll();
}

function renderCuinesActives() {
  const wrap = document.getElementById('cuines-actives');
  if (!state.cuines.length) { wrap.classList.add('hidden'); return; }
...
function bindEvents() {
  ...
  document.getElementById('filter-cuina').addEventListener('change', e => {
    if (e.target.value) { addCuina(e.target.value); e.target.value = ''; }
  });

  document.getElementById('clear-cuina').addEventListener('click', () => { state.cuines = []; renderAll(); });
  ...
  document.getElementById('pos-text').addEventListener('keydown', e => { if (e.key === 'Enter') searchPos(); });
