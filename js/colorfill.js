...
// ── CONSTANTS ─────────────────────────────────────────────────
const CF_NIVELLS = {
  petit: { mida: 12, colors: 5, taulers: 3, limit: 22, pts_base: 100, pts_mov: 25, top: 3 },
  mitja: { mida: 14, colors: 6, taulers: 4, limit: 25, pts_base: 100, pts_mov: 25, top: 4 },
  gran:  { mida: 16, colors: 7, taulers: 3, limit: 28, pts_base: 100, pts_mov: 25, top: 3 },
};
...
function colorfillGetPuntsGlobals() {
  const res = {};
  (window.JUGADORS_VALIDS || ['Jordi','Anna','Laia','Mons','Xu','Joa']).forEach(nom => {
  ...
  const html = `
    <div class="cf-selector-wrap">
      ...
      <div class="cf-nivells-grid">
        ${Object.entries(CF_NIVELLS).map(([key, cfg]) => cfRenderNivellCard(key, cfg)).join('')}
      </div>
    </div>
  `;
  cont.innerHTML = html;
...
function cfRenderJoc() {
  ...
  const { cfg, nivellKey, tauler, movsRestants, movsFets, acabat, guanyat, colorActual } = cfJoc;
  ...
  const movsColor = movsRestants <= 3 ? '#e63946' : movsRestants <= 6 ? '#f4a261' : '#a8d8b0';
...
function cfCalcularRegio(tauler) {
  ...
  while (cua.length > 0) {
    ...
    for (const [dr, dc] of [[-1,0],[1,0],[0,-1],[0,1]]) {
      ...
      if (nr >= 0 && nr < mida && nc >= 0 && nc < mida &&
          !visitat.has(`${nr},${nc}`) && tauler[nr][nc] === colorRegio) {
        visitat.add(`${nr},${nc}`);
        cua.push([nr, nc]);
      }
    }
